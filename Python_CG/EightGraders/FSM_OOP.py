...
def _get_trigger(model, trigger_name, *args, **kwargs):
    ...
    """
    print("In _get_trigger = %s, %s" %(model, trigger_name))
    ...
    def check(self, event_data):
        ...
        Args:
            event_data (EventData): An EventData instance to pass to the
                ...
                the condition.
        """
        if False: #isinstance(self.func, string_types):
           predicate = getattr(event_data.model, self.func)
           print("passing")
        else: predicate = self.func
...
class Transition(object):
    ...
    def execute(self, event_data):
        ...
        print("in Tranisition::Execute - BEFORE machine.callback")
        for func in itertools.chain(self.after, machine.after_state_change):
            print("in Tranisition::Execute - calling machine.callback")
            machine.callback(func, event_data)
            print("in Tranisition::Execute - calling machine.callback")
...
class Machine(object):
    ...
    def __init__(self, model='self', states=None, initial='initial', transitions=None,
        ...
        if ordered_transitions:
            print("adding ordered transitions")
            self.add_ordered_transitions()

        if model:
            print("adding player model")
        ...
        for mod in models:
            ...
            if mod not in self.models:
                if hasattr(mod, 'trigger'):
                    ...
                    #partial, calls _get_trigger with only mod parameter
                    mod.trigger = partial(_get_trigger, mod)
                    print("mod.trigger = ", mod.trigger)
                    #mod.trigger has player.
    ...
    def set_state(self, state, model=None):
        ...
        if True: #isinstance(state, string_types):
    ...
    def add_transition(self, trigger, source, dest, conditions=None,
        ...
        if True: #isinstance(source, string_types):
    ...
    def callback(self, func, event_data):
        ...
        """
        print("in Machine::Callback")
        if False:#isinstance(func, string_types):
        ...
        if self.send_event:
            ...
            print("calling the state callback")
            func(*event_data.args, **event_data.kwargs)

    def _has_state(self, state):
        if True: #isinstance(state, State):
    ...
    def _process(self, trigger):
        print("Trigger start_machine comes here")
        # default processing
