...
for i in range(4):
    ...
    k=cv2.imread(r'dilatedpart.jpg')
    ...
    hough_lines=cv2.imread(r'hough1.jpg')
    p=[]
    ...
    for i in range(len(pxl)):
        ...
        contours, hierarchy,_ = cv2.findContours(thresh.copy(),cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
        print(contours)
        print(len(contours))
        for cnt in contours:
            #print(cnt)
            ctr = np.array(cnt).reshape((-1,1, 23)).astype(np.int32)
            print(ctr)
